...
import NavbarAdmin from "../NavbarAdmin/page"; // Adjust path if needed
...
function UpdateUsersInner() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push("AllParticipants"); // Navigate back or refresh
      } else {
  ...
  if (loading && users.length === 0)
    return (
      <div>
        <NavbarAdmin /> <div className="p-4">Loading user data...</div>
      </div>
    ); // Show loading only initially
  if (error)
    return (
      <div>
        <NavbarAdmin /> <div className="p-4 text-red-500">Error: {error}</div>
      </div>
    );
...
export default function UpdateUsers() {
  return (
    ...
    <Suspense
      fallback={
        <div>
          <NavbarAdmin /> <div className="p-4">Loading...</div>
        </div>
      }
    >
      <UpdateUsersInner />
