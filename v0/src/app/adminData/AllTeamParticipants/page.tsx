...
export default function AllTeamParticipants() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (data.success) {
          ...
          setError(data.message || "Failed to fetch TeamParticipants.");
          toast.error(data.message || "Failed to fetch TeamParticipants.");
          router.push("/userData/LoginUser");
        }
      } catch (error) {
        console.error("Error fetching TeamParticipants:", error);
        setError("Failed to fetch TeamParticipants. Please try again later.");
      }
      setLoading(false);
    };
  ...
  const handleDelete = async () => {
    if (selectedUsers.length === 0) {
      alert("Please select at least one TeamParticipants to delete.");
      return;
    }

    const confirmDelete = confirm(
      "Are you sure you want to delete selected TeamParticipants?"
    );
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        alert("Selected TeamParticipants deleted successfully!");
        setUsers(users.filter((user) => !selectedUsers.includes(user.email)));
        setSelectedUsers([]); // Reset selection
      } else {
        alert("Failed to delete TeamParticipants: " + data.message);
      }
    } catch (error) {
      console.error("Error deleting TeamParticipants:", error);
      alert("Failed to delete TeamParticipants. Please try again.");
    }
  };
    ...
    <div>
      ...
      <div className="p-4">
        <h1 className="text-2xl font-bold mb-4">
          All Registered TeamParticipants
        </h1>
        ...
        <div className="mt-4 flex gap-4">
          ...
          >
            Delete Selected TeamParticipants
          </button>
          ...
          >
            Update Selected TeamParticipants
          </button>
        </div>
      </div>
    </div>
